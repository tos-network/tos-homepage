...
const translations = {
  en: {
    ...
    'features.referral.desc': 'Chain-native referral relationships with 11 syscalls for smart contracts. Multi-level upline queries, team volume tracking, and batch reward distribution. Purpose-built for viral marketing DApps with zero gas overhead for relationship storage.',
    'features.kyc.title': 'Chain-Native KYC Compliance',
    'features.kyc.desc': 'Minimal on-chain footprint (43 bytes/user) with 9-tier verification levels. Regional security committees manage off-chain compliance data. FATF Travel Rule and MiCA aligned. Privacy-preserving design keeps PII off-chain while enabling smart contract access control.',
    'features.qrpayment.title': 'QR Code Instant Payments',
    'features.qrpayment.desc': 'PayPay/Alipay-style QR payments with 3-second confirmation. Merchant-friendly RPC APIs, WebSocket subscriptions, and HMAC-secured webhooks. Complete payment lifecycle from request creation to blockchain-verified settlement.',
  ...
  zh: {
    ...
    'features.referral.desc': '链原生推荐关系存储,11个智能合约系统调用。多层上级查询、团队业绩追踪、批量分润分发。专为裂变营销DApp设计,推荐关系存储零Gas开销。',
    'features.kyc.title': '链原生KYC合规',
    'features.kyc.desc': '极简链上存储(43字节/用户),9级验证等级。区域安全委员会管理链下合规数据。符合FATF旅行规则和MiCA标准。隐私保护设计将PII数据保留在链下,同时支持智能合约访问控制。',
    'features.qrpayment.title': 'QR码即时支付',
    'features.qrpayment.desc': '类似PayPay/支付宝的二维码支付,3秒确认。商户友好的RPC API、WebSocket订阅和HMAC安全回调。从请求创建到区块链验证结算的完整支付生命周期。',
  ...
  ja: {
    ...
    'features.referral.desc': 'チェーンネイティブの紹介関係ストレージと11のスマートコントラクトシステムコール。マルチレベルアップライン照会、チームボリューム追跡、バッチ報酬分配。バイラルマーケティングDApp向けに設計、紹介関係ストレージはゼロガスオーバーヘッド。',
    'features.kyc.title': 'チェーンネイティブKYCコンプライアンス',
    'features.kyc.desc': '最小限のオンチェーンフットプリント（43バイト/ユーザー）と9段階の検証レベル。地域セキュリティ委員会がオフチェーンコンプライアンスデータを管理。FATFトラベルルールとMiCA準拠。プライバシー保護設計でPIIをオフチェーンに保持しながらスマートコントラクトアクセス制御を実現。',
    'features.qrpayment.title': 'QRコード即時決済',
    'features.qrpayment.desc': 'PayPay/Alipayスタイルの3秒確認QR決済。マーチャントフレンドリーなRPC API、WebSocketサブスクリプション、HMAC保護付きWebhook。リクエスト作成からブロックチェーン検証済み決済までの完全な支払いライフサイクル。',
  ...
  ko: {
    ...
    'features.referral.desc': '체인 네이티브 추천 관계 저장소와 11개의 스마트 컨트랙트 시스템 호출. 다중 레벨 업라인 조회, 팀 볼륨 추적, 배치 보상 배포. 바이럴 마케팅 DApp을 위해 설계, 추천 관계 저장소 가스 오버헤드 제로.',
    'features.kyc.title': '체인 네이티브 KYC 컴플라이언스',
    'features.kyc.desc': '최소한의 온체인 풋프린트(43바이트/사용자)와 9단계 검증 레벨. 지역 보안 위원회가 오프체인 컴플라이언스 데이터 관리. FATF 여행 규칙 및 MiCA 준수. 프라이버시 보호 설계로 PII를 오프체인에 유지하면서 스마트 컨트랙트 접근 제어 지원.',
    'features.qrpayment.title': 'QR 코드 즉시 결제',
    'features.qrpayment.desc': 'PayPay/Alipay 스타일의 3초 확인 QR 결제. 가맹점 친화적인 RPC API, WebSocket 구독, HMAC 보안 웹훅. 요청 생성부터 블록체인 검증 정산까지 완전한 결제 라이프사이클.',
